...
.rec-box.teal   { background: #f0fdfa; border-color: #14b8a6; color: #134e4a; }
...
<div class="app-header">
  ...
  <div>
    ...
    <p class="app-header-sub">DKD Clinical Decision Support &nbsp;·&nbsp; Multimodel Ensemble · KDIGO 2024 Staging · Fairness-Audited · Explainable AI</p>
  </div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    <span style="color:#60a5fa;font-weight:600">Clinical Model (M1)</span> — 53.1%<br>
    Blood tests and kidney markers<br><br>
    <span style="color:#a78bfa;font-weight:600">Demographics Model (M3)</span> — 27.3%<br>
    Age, sex, ethnicity, history<br><br>
    <span style="color:#fb923c;font-weight:600">Lifestyle Model (M2)</span> — 19.6%<br>
    Medications, activity, diet<br><br>
    <span style="color:#94a3b8;font-weight:600">Combined Decision</span><br>
    Meta-learner weights from SHAP analysis
    </div>
    """, unsafe_allow_html=True)
    st.markdown("---")
    st.markdown("""
    <div style="font-size:.7rem;color:#475569;text-align:center">
    NHANES 2015–2020 · n=2,627 · AUC 0.961
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.current_page == 0:
    ...
    # Store for use in pages 5 and 6
    st.session_state["last_pred"]   = pred_stage
    st.session_state["last_proba"]  = proba_vec
    st.session_state["last_hba1c"]  = st.session_state.patient_data.get("hba1c_pct", 7.2)
    ...
    <div class="disclaimer">
      ...
      (2015–2020, n=2,627); not validated for longitudinal monitoring without repeated assessment.
    </div>""", unsafe_allow_html=True)
    ...
    <div style="margin-bottom:1.2rem">
      ...
      <div style="font-size:.84rem;color:#64748b;line-height:1.7">
        Uses the actual trained GlomeraAI ensemble to show how predicted kidney disease risk
        changes as HbA1c improves month by month. Every result is a real model prediction.
        Complete the patient assessment first to use your patient's data.
      </div>
    </div>
    """, unsafe_allow_html=True)

    has_patient = bool(st.session_state.patient_data)
    current_hba1c = st.session_state.patient_data.get("hba1c_pct", 9.0)

    if not has_patient:
        st.markdown("""
        <div class="rec-box amber">
          No patient loaded. Complete the Patient Assessment first, or adjust the sliders
          below to explore a hypothetical scenario.
        </div>""", unsafe_allow_html=True)
    ...
    with col_l:
        hba1c_now  = st.slider("Current HbA1c (%)", 5.5, 14.0,
                                float(round(current_hba1c, 1)), step=0.1)
        hba1c_goal = st.slider("Target HbA1c at end of projection (%)", 5.5, 14.0,
    ...
    if hba1c_goal >= hba1c_now:
        ...
        <div class="rec-box red">
          Target HbA1c must be lower than the current value to model an improvement.
        </div>""", unsafe_allow_html=True)
    else:
        ...
        if st.button("▶  Run What-If Analysis", type="primary"):
            if not has_patient:
                base = {
                    ...
                    "log_uacr": np.log10(45.0), "hemoglobin_gdl": 13.0,
                    "hematocrit_pct": 39.0, "serum_albumin_gdl": 4.0,
                    "bun_mgdl": 18.0, "log_bun_mgdl": np.log1p(18.0),
                    "uric_acid_mgdl": 5.8, "crp_mgL": 4.0, "log_crp_mgL": np.log1p(4.0),
                    "total_cholesterol_mgdl": 210.0, "ldl_cholesterol_mgdl": 130.0,
                    "hdl_cholesterol_mgdl": 42.0, "triglycerides_mgdl": 188.0,
                    "log_triglycerides_mgdl": np.log1p(188.0), "bmi_kgm2": 31.0,
                    "fasting_glucose_mgdl": 178.0, "log_fasting_glucose_mgdl": np.log1p(178.0),
                    "insulin_uiml": 16.0, "log_insulin_uiml": np.log1p(16.0),
                    "hypertension_diagnosed": 1, "kidney_disease_history": 0,
                    "kidney_stone_history": 0, "insulin_use": 0, "bp_medication": 1,
                    "diabetes_diagnosed": 1, "oral_diabetes_meds": 1, "statin_use": 1,
                    "current_smoker_status": 0, "avg_alcohol_drinks_per_day": 0.1,
                    "log_avg_alcohol_drinks_per_day": np.log1p(0.1),
                    "vigorous_leisure_activity": 0, "sedentary_minutes_per_day": 360,
                    "log_sedentary_minutes_per_day": np.log1p(360), "sleep_hours_weekday": 6.5,
                    "nocturia": 1, "sodium_mg_day": 3100.0, "protein_g_day": 90.0,
                    "potassium_mg_day": 2600.0, "phosphorus_mg_day": 1060.0,
                    "age_years": 55, "sex_code": 1, "race_ethnicity_code": 4,
                    "education_level": 3, "household_income_cat": 6,
                    "food_security_score": 9.0, "coronary_heart_disease": 0,
            ...
            with st.spinner(f"Running GlomeraAI at {n_months + 1} time points..."):
                for t, hba1c_val in enumerate(trajectory):
                    pt = dict(base)
                    pt["hba1c_pct"] = float(hba1c_val)
                    pt["log_fasting_glucose_mgdl"] = np.log1p(
                        base.get("fasting_glucose_mgdl", 178.0) * (hba1c_val / hba1c_now)
                    )
                    _, X1t, X2t, X3t = build_patient_vector(pt, mdl)
                    pred_t, proba_t, _, _, _ = run_ensemble(mdl, X1t, X2t, X3t)
                    results_over_time.append({
                        "month": t, "hba1c": hba1c_val,
                        "pred": pred_t, "proba": proba_t.tolist(),
                    })
            ...
            sev_start   = sum(c * start_proba[c] for c in range(N_CLASSES))
            sev_end     = sum(c * end_proba[c]   for c in range(N_CLASSES))
            delta       = sev_end - sev_start

            c1, c2, c3, c4 = st.columns(4)
            with c1:
                st.markdown(f'<div class="metric-tile"><div class="value" style="color:{STAGE_COLORS[start_pred]}">Stage {start_pred}</div><div class="label">Predicted now</div></div>', unsafe_allow_html=True)
            with c2:
                st.markdown(f'<div class="metric-tile"><div class="value" style="color:{STAGE_COLORS[end_pred]}">Stage {end_pred}</div><div class="label">At {n_months} months</div></div>', unsafe_allow_html=True)
            with c3:
                st.markdown(f'<div class="metric-tile"><div class="value" style="color:#f59e0b;font-size:1.3rem">{hba1c_now:.1f}→{hba1c_goal:.1f}%</div><div class="label">HbA1c trajectory</div></div>', unsafe_allow_html=True)
            with c4:
            ...
            if delta < -0.3:
                ...
                <div class="rec-box green">
                  <strong>Clinically meaningful improvement projected.</strong>
                  Reducing HbA1c from <strong>{hba1c_now:.1f}%</strong> to <strong>{hba1c_goal:.1f}%</strong>
                  via {intervention_label.lower()} is associated with a severity score decrease
                  of <strong>{abs(delta):.2f} points</strong> in the model.
                </div>""", unsafe_allow_html=True)
            elif delta < 0:
                st.markdown(f"""
                <div class="rec-box amber">
                  <strong>Modest improvement projected.</strong>
                  Severity score decreases by <strong>{abs(delta):.2f} points</strong>.
                  Additional interventions may amplify the benefit.
                </div>""", unsafe_allow_html=True)
            else:
                st.markdown("""
                <div class="rec-box blue">
                  <strong>Other risk factors are dominant.</strong>
                  HbA1c control alone shows limited impact for this patient profile.
                  Review blood pressure, proteinuria, and renal biomarkers.
                </div>""", unsafe_allow_html=True)

            months       = [r["month"] for r in results_over_time]
            sev_scores   = [sum(c * r["proba"][c] for c in range(N_CLASSES)) for r in results_over_time]
            hba1c_vals   = [r["hba1c"] for r in results_over_time]
            proba_matrix = np.array([[r["proba"][c] for c in range(N_CLASSES)]
                                     for r in results_over_time])

            fig, axes = plt.subplots(1, 3, figsize=(14, 4))

            axes[0].plot(months, hba1c_vals, color="#f59e0b", lw=2.5, marker="o", markersize=4)
            axes[0].axhline(7.0, color="#22c55e", lw=1, ls="--", alpha=0.7, label="Target <7%")
            axes[0].fill_between(months, hba1c_vals, 7.0,
                                 where=[h > 7.0 for h in hba1c_vals], alpha=0.12, color="#ef4444")
            axes[0].set_title("HbA1c Trajectory", fontsize=10, fontweight="700")
            axes[0].set_xlabel("Month"); axes[0].set_ylabel("HbA1c (%)")
            axes[0].legend(fontsize=8); axes[0].spines[["top", "right"]].set_visible(False)

            axes[1].plot(months, sev_scores, color="#3b82f6", lw=2.5, marker="o", markersize=4)
            axes[1].fill_between(months, sev_scores, alpha=0.1, color="#3b82f6")
            for t_pt in [0, len(months) - 1]:
                axes[1].annotate(f"S{results_over_time[t_pt]['pred']}",
                                 (months[t_pt], sev_scores[t_pt]),
                                 textcoords="offset points", xytext=(0, 8),
                                 fontsize=8, ha="center",
                                 color=STAGE_COLORS[results_over_time[t_pt]["pred"]],
                                 fontweight="bold")
            axes[1].set_title("Severity Score (Real Model Output)", fontsize=10, fontweight="700")
            ...
            for c in range(N_CLASSES):
                axes[2].fill_between(months, bottom, bottom + proba_matrix[:, c],
                                     color=STAGE_COLORS[c], alpha=0.82,
                                     label=STAGE_NAMES[c].split("—")[0].strip())
                bottom += proba_matrix[:, c]
            axes[2].set_title("Stage Probability Over Time", fontsize=10, fontweight="700")
    ...
    st.markdown("""
    <div class="disclaimer">
      <strong>What this tool shows:</strong> Each result is a real prediction from the trained GlomeraAI
      ensemble re-run with a modified HbA1c value. This is a sensitivity analysis — it shows the model's
      response to hypothetical HbA1c changes, not a prediction of what will clinically happen over time.
      GlomeraAI was trained on cross-sectional NHANES 2015–2020 data and cannot model longitudinal
      trajectories. All outputs require clinician review before informing any clinical decision.
    </div>""", unsafe_allow_html=True)
